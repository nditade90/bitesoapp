...
class Crud extends MX_Controller {
	...
	public function index() {
		...
		$this->load->view('crudv1/index', $data);
	}
...
//			die;
			if (!isset($primary_key)) {
				...
				redirect(base_url('crudv1/Crud/index'));
			}
...
//set some basic validation rules
		...
		foreach ($fields as $field) {
			if ($field->name == 'created') {
				$controller_code .= "\t\t\t\$this->input->post('created') = time();\n";
			} elseif ($field->name == 'modified') {
				$controller_code .= "\t\t\t\$this->input->post('modified') = 0;\n";
		...
		$controller_code .= "
				...
				if(!empty(\$insert)){
					\$this->session->set_flashdata('msg','<div class=\"text-success\">'.\$this->input->post().' a été enregistré avec succès.</div>');
				}else{
					\$this->session->set_flashdata('msg','<div class=\"text-danger\">L\'enregistrement du '.\$this->input->post().' a échoué </div');
				}
			redirect(base_url('" .$getcontroller->module."/". $getcontroller->controller . "/add'));
			}
...
//some basic validations
		...
		public function delete(\$id){
			\$data = \$this->My_model->get_one('".$table."',array('".$primary_key."'=>\$id));

			if(\$this->db->delete('" . $table . "',array('" . $primary_key . "'=>\$id))){
				\$this->session->set_flashdata('msg','<div class=\"text-success\"> $ a été supprimée.</div>');
				redirect(base_url('".$getcontroller->module ."/" . ucfirst($getcontroller->controller) . "/add'));
			}
		}
}
...
/*
    ...
    <section class=\"content\">
		...
		<?=form_open('" . $getcontroller->module."/".$getcontroller->controller. "/index')?>
		\n\n";
		foreach ($fields as $field) {
			...
			if ($field->primary_key != '1' && $field->name != "created" && $field->name != 'modified') {
				$add_view_code .= "\t\t\t<div class='form-group'><label>" . $field->name . "</label>\n";
				if(!empty($foreign)){
		...
		$view_view = $add_view_code."<div class='col-md-8'>";
		$view_view .= $this->generate_index_view($primary_key, $table, $fields)."</div></div></section></div>";
		if (!write_file($add_view, $view_view)) {
			return false;
		} else {
			return true;
		}
	...
	protected function generate_edit_view($primary_key, $table, $fields) {
		...
		foreach ($fields as $field) {
			...
			if ($field->primary_key != '1' && $field->name != 'created' && $field->name != 'modified') {
				$edit_view_code .= "\t\t\t<div class='form-group'><label>" . $field->name . "</label><?php echo form_error('" . $field->name . "'); ?>\n";
		...
		$view_view = $edit_view_code."<div class='col-md-8'>".$this->generate_index_view($primary_key, $table, $fields)."</div></div></section></div>";
		
		if (!write_file($edit_view, $view_view)) {
			return false;
		} else {
			return true;
		}
