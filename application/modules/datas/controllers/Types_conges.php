...
class Types_conges extends Admin_Controller{
	...
	public function index(){
		...
		public function add(){
			...
			$config[ 'base_url' ]      = base_url( 'datas/Types_conges/index' );
			$config[ 'per_page' ]      = 10;
			$config[ 'num_links' ]     = 2;
			...
			$this->form_validation->set_rules('nb_jour', 'Nb_jour', 'required|numeric');
			...
			if($this->form_validation->run()){
				...
				if(!empty($insert)){
					$this->session->set_flashdata('msg','<div class="text-success">Type de congé a été enregistré avec succès.</div>');
				}else{
					$this->session->set_flashdata('msg','<div class="text-danger">L\'enregistrement d\'un type de congé a échoué </div');
				}
			redirect(base_url('datas/Types_conges/add'));
			}
		...
		public function view($id){
			...
			$this->data[ 'title' ] = 'AddTypes_conges';
		...
		public function edit(){
			...
			$this->db->where('id_type_conge',$id);
					...
					if(!empty($insert)){
						$this->session->set_flashdata('msg','<div class="text-success">Type de congé a été mis a jour avec succès.</div>');
					}else{
						$this->session->set_flashdata('msg','<div class="text-danger">La mis a jour d\'un type de congé a échoué </div');
					}
				redirect(base_url('datas/Types_conges/add'));
				}
		...
		public function delete($id){
			...
			if($this->db->delete('mv_types_conges',array('id_type_conge'=>$id))){
				$this->session->set_flashdata('msg','<div class="text-success">Type de congé a été supprimée.</div>');
				redirect(base_url('datas/Types_conges/add'));
			}
		}
}
